	...
	void ct_command()
	{
		...
		while (true) {
			...
			// cout << name() << "::ct_command ts: " << sc_time_stamp() << ", dmac: " << dmac << endl;
			{
				...
				unsigned bsize = MAX_BLOCK_SIZE; // block size in bytes
				...
				while (size) {
					...
					tpay->set_data_length((tsize < 16) ? 16 : tsize); // HMCSim seg faults if size < 16
	...
	void ct_response()
	{
		...
		while (true) {
			...
			while (len--) {
				...
				flit = s_dat.read();
				// if (strstr(name(),"lsu2") != NULL)
				// cout << name() << "::nb_transport_bw ts: " << sc_time_stamp() << ", flit: " << flit << endl;
				for (int i = 0; i < (DD/8); i += sizeof(word)) {
