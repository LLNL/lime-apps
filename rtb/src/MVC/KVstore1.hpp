...
public:
	...
	bool _get(key_type key, char *&value)
	{
	...
	bool _put(key_type key, char *&value)
	{
		size_t idx = hash_idx(key);
		size_t itop = idx + topsearch;
...
public:
	...
	bool _scan(slot_s *slot, size_t imax, key_type key, char *&value)
	{
		size_t idx = 0;
		/* search for key */
	...
	void fill(void *buf, size_t buf_sz, size_t offset)
	{
		...
		while (idx < n) {
			...
			memcpy(bptr, dptr, elem_sz); // TODO: is assignment faster?
			idx++;
			bptr += elem_sz;
		}
	}
	...
	void drain(void *buf, size_t buf_sz, size_t offset)
	{
		...
		while (idx < n) {
			...
			memcpy(dptr, bptr, elem_sz); // TODO: is assignment faster?
			idx++;
			bptr += elem_sz;
		}
	}
