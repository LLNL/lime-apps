...
tick_t t0, t1;
#if defined(_OPENMP)
#pragma omp threadprivate(t0,t1)
#endif
tick_t start, finish;
...
#if defined(NO_BUFFER) || defined(_OPENMP)

	inline void lookup(key_type key)
	{
		// TODO: timer seems off under _OPENMP, try a local t0
		tget(t0);
#if defined(MULTIMAP)
...
MAIN
{
	...
	if (warg && kdb.size()) {
		...
		pcount--;
		tget(start);
#if defined(USE_ACC) || !defined(_OPENMP)
...
#endif
		for (lcount = 0; lcount < warg; lcount++) {
