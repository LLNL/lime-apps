...
sub scan {
	...
	while (<$fhi>) {
		...
		if (/Real time used = (\d+\.\d+)/) { # random access
			$tref->{$q_del}{$t_del}{RUN} = $1; # seconds
			next;
		}
		if (/(\d+\.\d+) Billion/) {
			$tref->{$q_del}{$t_del}{GUPS} = $1; # GUP/s
			next;
		}
		if (/Oper\. time: (\d+\.\d+)/) {
...
print "\n";
print "RandomAccess Run Time (seconds) - Stock Algorithm\n";
...
print "\n";
print "RandomAccess Run Time (seconds) - View Buffer Algorithm\n";
print "Queue Delay / Link Delay";
...
print "\n";
print "RandomAccess Giga Updates per Second (GUP/s) - Stock Algorithm\n";
print "Queue Delay / Link Delay";
foreach my $td (sort keys %{$tab{ST}{(sort keys %{$tab{ST}})[0]}}) {
	print ", ", $td;
}
...
foreach my $qd (sort keys %{$tab{ST}}) {
	print $qd;
	foreach my $td (sort keys %{$tab{ST}{$qd}}) {
		print ", ", $tab{ST}{$qd}{$td}{GUPS};
	}
	print "\n";
}

print "\n";
print "RandomAccess Giga Updates per Second (GUP/s) - View Buffer Algorithm\n";
print "Queue Delay / Link Delay";
foreach my $td (sort keys %{$tab{VB}{(sort keys %{$tab{VB}})[0]}}) {
	print ", ", $td;
}
...
foreach my $qd (sort keys %{$tab{VB}}) {
	print $qd;
	foreach my $td (sort keys %{$tab{VB}{$qd}}) {
		print ", ", $tab{VB}{$qd}{$td}{GUPS};
	}
	print "\n";
}

print "\n";
print "RandomAccess Speedup\n";
print "ST_RUN/VB_RUN\n";
print "Queue Delay / Link Delay";
foreach my $td (sort keys %{$tab{ST}{(sort keys %{$tab{ST}})[0]}}) {
	print ", ", $td;
...
print "\n";
print "RandomAccess Speedup Upper Bound\n";
...
print "\n";
print "RandomAccess CPU Data Transferred (bytes) - Stock Algorithm\n";
my $ST_CPU_bytes = 0;
my $ST_CPU_count = 0;
foreach my $qd (sort keys %{$tab{ST}}) {
	foreach my $td (sort keys %{$tab{ST}{$qd}}) {
...
print "\n";
print "RandomAccess CPU Data Transferred (bytes) - View Buffer Algorithm\n";
my $VB_CPU_bytes = 0;
my $VB_CPU_count = 0;
foreach my $qd (sort keys %{$tab{VB}}) {
	foreach my $td (sort keys %{$tab{VB}{$qd}}) {
